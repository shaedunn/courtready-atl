...
export default function ConditionReportFlow({ courtId, variant = "primary" }: ConditionReportFlowProps) {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (reportId) => {
      ...
      setTimeout(() => setButtonLabel("📍 I'm here — report conditions"), 3000);
    },
  });
  ...
  const tagMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      confirmTimer.current = setTimeout(() => setTagConfirmation(false), 2500);
    },
  });

  const handleConditionTap = useCallback((condition: Condition) => {
    setDrawerOpen(false);
    submitMutation.mutate({ condition });
  }, [submitMutation]);
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Effort tag pills — only after a report this session */}
